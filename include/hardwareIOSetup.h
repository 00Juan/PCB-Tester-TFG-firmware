...
ChannelCalibrationData chCalData[8] = {
    ...
    { 4.922408941f, -3.92006689f, 0.001811177f,   0.005067654f, 0.022896532f, 821.5434084, 50.503215434 }, // CH1 //estaba a -1.503215434
 { 4.922408941f, -3.92006689f, 0.001811177f,   0.005067654f, 0.022896532f, 821.5434084, 58.503215434 }, // CH2
    { 4.922409058f, -3.922501564f, 0.198214382f, 0.005072452f, 0.014393775f, 821.543395996f, 0.0f }, // CH3
    { 4.922409058f, -3.874722004f, 0.151402324f, 0.005054527f, 0.027935842f, 821.543395996f, 0.0f }, // CH4
    { 4.922409058f, -3.913446903f, 0.668581843f, 0.005076871f, 0.020646263f, 821.543395996f, 0.0f }, // CH5
    { 4.922409058f, -3.903074980f, 0.640818477f, 0.005070274f, 0.020673038f, 821.543395996f, 0.0f }, // CH6
    { 4.922409058f, -3.891319752f, 0.623539805f, 0.005136022f, 0.022460615f, 821.543395996f, 0.0f }, // CH7
    { 4.922409058f, -3.876799345f, 0.617106318f, 0.005073663f, 0.024549371f, 821.543395996f, 0.0f }, // CH8
};
...
bool initializeMCP3208()
{
    ...
    for (uint8_t i = 0; i < numCsPins; i++)
    {
        ...
        // Command byte: Start bit (1) + Single-ended (1) + Don't care (0) + Channel selection (3 bits)
        // For channel 0: 0x06 (binary: 0000 0110)
        ...
        uint8_t dataHigh = SPI.transfer(0x00);        // Receive high byte (12-bit value MSB)
        ...
        {
            Serial.printf("Status: ⚠ Channel floating high (no connection or Vref issue)\n");
        }
        ...
        {
            deviceCount++;
            Serial.printf("Result: ✓ MCP3208 detected and responding at CS Pin %d (%s)\n", csPin, csNames[i]);
        }
        else if (validRange1 && validRange2)
        {
            Serial.printf("Result: ⚠ Possible MCP3208 with floating input or reference issue\n");
        }
        else
        {
 ...
 * Tests both ADC chip select pins (ADC1 and ADC2) with MCP3208-specific diagnostics.
 *
...
uint8_t scanSPIDevices()
{
    ...
    for (uint8_t i = 0; i < numCsPins; i++)
    {
        ...
        uint16_t valueDifference = (adcValue1 > adcValue2) ? (adcValue1 - adcValue2) : (adcValue2 - adcValue1);
        ...
        // Determine if MCP3208 is detected
        bool mcp3208Detected = validRange1 && validRange2 && consistent && notFloating;
